...
class Place(object):
    ...
    def __init__(self, name, exit=None):
        ...
        if self.exit is not None:
            self.exit.entrance = self # 一块地的出口的入口就是它自己
        # END Problem 2
...
class Ant(Insect):
    ...
    def can_contain(self, other):
        # BEGIN Problem 9
        "*** YOUR CODE HERE ***"
        if self.container == True and other.container == False and self.ant is None:
            return True
        else:
            return  False
...
class HarvesterAnt(Ant):
    ...
    name = 'Harvester'
    implemented = True
    food_cost = 2  # 初始化一个HarvestAnt,消耗食物为2
    armor = 1   # 初始化一个HarvestAnt,armor为1
...
class ThrowerAnt(Ant):
    ...
    implemented = True
    damage = 1
    food_cost = 3   # 初始化一个ThrowerAnt,消耗食物为3
    armor = 1  # 初始化一个ThrowerAnt,armor为1
    min_range = 0
    max_range = float('inf')
    ...
    def nearest_bee(self, hive):
        ...
        while place != hive:
            ...
            if pos >= self.min_range and pos <= self.max_range and bee is not None:
                return bee
...
class AntColony(object):
    ...
    def __init__(self, strategy, hive, ant_types, create_places, dimensions, food=10):
